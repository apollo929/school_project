...
function Results({
  ...
  const getCustomers = async () => {
    ...
    axios.post(`${API_BASE_URL}/user/load`, data, {
  ...
  const processAfterDeleteDone = (rtn_status) => {
    if (rtn_status) {
      enqueueSnackbar('Teachers deleted', {
        variant: 'success',
        action: <Button>See all</Button>
      });
    } else {
      enqueueSnackbar('Error Occured! Can not delete Teachers.', {
        variant: 'error',
        action: <Button>See all</Button>
      });
    }
  ...
  const processConfirmBulkDelete = async () => {
    asyncForEach(selectedCustomers, async (selectedCustomer) => {
      const response = await axios.delete(`${API_BASE_URL}/user/${selectedCustomer}`);
                ...
                >
                  <Box>
                    ...
                    <Box className={classes.noDataTextWrapper}>
                      ...
                      >
                        There are no teachers in your account.
                      </Typography>
                    </Box>
                  </Box>
                </Box>
                      ...
                      <TableCell>
                        ...
                        >
                          <Avatar
                            ...
                            src={('http://localhost:5000' + customer.avatar.replace("public", ""))}
                          >
